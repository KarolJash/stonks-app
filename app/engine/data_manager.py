...
def insert_on_conflict(table, conn, keys, data_iter):
    ...
    on_conflict_stmt = stmt.on_conflict_do_nothing(index_elements=['date', 'ticker'])
...
def import_from_db(ticker, table, start, end):
    query = select(table).where(table.ticker == ticker).where(table.date > start).where(table.date < end)
