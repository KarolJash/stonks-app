...
from app.engine.main import train_xgboost
from app.dependencies import db_validate_ticker, real_ticker

from app.schemas import TickerRequest, DelistCheckRequest, DelistAddRequest, XgboostTrainingRequest

app = FastAPI()

@app.post("/download/stock", status_code=status.HTTP_202_ACCEPTED)
async def download_stock(payload: TickerRequest, background_tasks: BackgroundTasks, valid_ticker: str = Depends(real_ticker)):
...
async def download_market(background_tasks: BackgroundTasks):
    ...
    return {"message": f'Task to update index infromation accpeted. Check status later.'}

@app.post("/delist/add", status_code=status.HTTP_202_ACCEPTED)
async def add_delisted_func(payload: DelistAddRequest, background_tasks: BackgroundTasks):
    background_tasks.add_task(add_delisted, payload.ticker)

    return {"message": f"Task to add {payload.ticker} to delisted securities accepted. Check status later."}
...
async def delisted_func(payload: DelistCheckRequest, background_tasks: BackgroundTasks):
    ...
    return {"message": f"Task to check delisted securities accpeted. Check status later."}

@app.post("/xgboost/train", status_code=status.HTTP_202_ACCEPTED)
async def xgboost_train(payload: XgboostTrainingRequest, background_tasks: BackgroundTasks, valid_ticker: str = Depends(db_validate_ticker)):
